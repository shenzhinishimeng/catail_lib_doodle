...
public class DoodlePath extends DoodleRotatableItemBase {
    ...
    private final PointF mSxy = new PointF(); // 映射后的起始坐标，（手指点击）
    private final PointF mDxy = new PointF(); // 映射后的终止坐标，（手指抬起）
    ...
    private final Matrix mBitmapColorMatrix = new Matrix();
    ...
    private static final WeakHashMap<IDoodle, HashMap<Integer, Bitmap>> sMosaicBitmapMap = new WeakHashMap<>();
